...
> [!TIP]
> `age` is much simpler than GPG — no keyservers, no trust model, no config files. Ideal for new projects.
> `age` гораздо проще GPG — нет серверов ключей, модели доверия или конфигов. Идеален для новых проектов.
