...
# 📅 date & timedatectl — Time Management Cheatsheet

> **Context:** Essential commands for managing system time, dates, timezones, and NTP synchronization. / Основные команды для управления системным временем, датами, часовыми поясами и синхронизацией NTP.
> **Role:** Sysadmin / DevOps
> **Tools:** date, timedatectl, chronyc, systemd-timesyncd
...
> [!IMPORTANT]
> `date -d` is a GNU coreutils extension and **does not work on macOS**. Use `gdate` (from `coreutils` Homebrew package) on macOS. / `date -d` не работает на macOS — используйте `gdate`.
