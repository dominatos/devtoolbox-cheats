...
> **Description:** Git is a free and open-source distributed version control system designed to handle everything from small to very large projects with speed and efficiency. Created by Linus Torvalds in 2005, it is the most widely used VCS in the world, used by virtually every modern software project.
> Git — это бесплатная распределённая система контроля версий с открытым исходным кодом, созданная Линусом Торвальдсом в 2005 году. Самая популярная VCS в мире.

> **Status:** Actively maintained, industry standard. No modern alternatives have replaced Git — GitHub, GitLab, Bitbucket all use Git as their backend.
> **Role:** Developer / DevOps / Sysadmin
> **Version:** 2.x+
...
| File | Scope (EN / RU) |
|------|-----------------|
| `/etc/gitconfig` | System-wide config / Системная конфигурация |
...
|----------|---------------------|---------|
| `merge` | Shared branches, preserving context / Общие ветки, сохранение контекста | Non-linear, preserves branches |
| `merge --no-ff` | Feature merges to main / Слияние фич в main | Explicit merge commit |
| `merge --squash` | Clean single commit for feature / Один коммит для фичи | Linear, loses branch detail |
| `rebase` | Local cleanup before push / Локальная очистка перед push | Linear, clean history |
...
|------|-------------------|-------------|---------------------|
| `--soft` | Unchanged | Unchanged | Redo commit message / Переделать сообщение коммита |
...
> [!TIP]
> Use `git revert` instead of `git reset` for commits that have been pushed to shared repositories. Revert creates a new commit, preserving history.
> Используйте `git revert` вместо `git reset` для коммитов, которые уже были отправлены в общие репозитории.
...
|------|----------|----------------------|
| 22 | SSH | Git over SSH / Git через SSH |
| 443 | HTTPS | Git over HTTPS / Git через HTTPS |
| 9418 | Git protocol | Native Git protocol (read-only, rare) / Нативный Git протокол |
