...
### Restore from Snapshot — Production Runbook
...
# Incremental send (-p specifies parent snapshot)
...
### Snapshot-Based Backup Workflow (LVM)

```bash
# 1. Create snapshot
lvcreate -L 5G -s -n backup_snap /dev/vg0/lv_data

# 2. Mount read-only and archive
...
# Remove oldest snapshots beyond retention count
...
```bash
# Error: "Insufficient free space" / "Недостаточно
...
```bash
# Error: "Cannot delete subvolume" / "Невозможно
...
```bash
# LVM — snapshot space usage / LVM —
lvs -o lv_name,data_percent                       # Usage in % / Использование в %

# ZFS — per-snapshot space usage / ZFS —
...
# Btrfs — overall + quota groups / Btrfs —
