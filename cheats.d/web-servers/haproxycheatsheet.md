...
**HAProxy** (High Availability Proxy) is a free, open-source, high-performance TCP/HTTP load balancer and reverse proxy. Created by Willy Tarreau in 2000, it is widely used in production environments for its reliability, efficiency, and advanced traffic management features.

**Common use cases / Типичные сценарии:**
- Layer 4 (TCP) and Layer 7 (HTTP) load balancing / Балансировка на уровнях L4 и L7
- SSL/TLS termination / Терминация SSL/TLS
- Rate limiting and DDoS protection / Ограничение скорости и защита от DDoS
- Blue-green and canary deployments / Blue-green и канареечные деплои
- High-availability reverse proxy / Высокодоступный обратный прокси
- WebSocket proxying / Проксирование WebSocket

> [!NOTE]
> HAProxy is the industry standard for high-performance load balancing. It is actively developed with both a **Community Edition** (open source) and **HAProxy Enterprise** (commercial). Alternatives include **Nginx** (general-purpose reverse proxy), **Envoy** (service mesh/L7 proxy), and **Traefik** (cloud-native, auto-discovery).
> HAProxy — отраслевой стандарт высокопроизводительной балансировки. Альтернативы: **Nginx**, **Envoy** (service mesh), **Traefik** (cloud-native).
...
| :--- | :--- | :--- |
| **How it works** | HAProxy periodically probes the server. | HAProxy monitors real traffic for errors. |
| **Как работает** | HAProxy периодически опрашивает сервер. | HAProxy мониторит реальный трафик. |
| **Detection** | Detects issues before traffic is affected. | Detects issues only after failed requests. |
...
- [HAProxy Starter Guide](https://www.haproxy.com/documentation/haproxy-configuration-tutorials/core-concepts/overview/)
- [HAProxy Runtime API](https://www.haproxy.com/documentation/haproxy-runtime-api/)
- [HAProxy Load Balancing Algorithms](https://www.haproxy.com/documentation/haproxy-configuration-tutorials/load-balancing/overview/)
- [HAProxy SSL/TLS Termination](https://www.haproxy.com/documentation/haproxy-configuration-tutorials/ssl-tls/)
