...
# Uyuni Sysadmin Cheatsheet

> **Context:** Uyuni is an open-source configuration management and infrastructure management solution for software-defined infrastructure. It is the community upstream project for SUSE Manager. / Uyuni — открытое решение для управления конфигурацией и инфраструктурой программно-определяемых сред. Является комьюнити-проектом, на основе которого создан SUSE Manager.
> **Role:** Sysadmin / DevOps / Infrastructure Engineer
> **Version:** Uyuni 2024.x (openSUSE Leap 15.x base)
> **Default Ports:** Web UI: `443` (HTTPS), `80` (HTTP redirect) | Salt Publisher: `4505` | Salt Return: `4506` | Jabber: `5222`, `5269` | OSAD: `5222`
...
3. [Salt & Configuration Management](#3-salt--configuration-management)
...
> [!IMPORTANT]
> Uyuni requires openSUSE Leap 15.x as the host OS. It cannot be installed on RHEL/Debian directly. Use the official container or VM image for other platforms.
...
| Method | Flag | Example / Пример | Description |
|--------|------|---------|-------------|
...
| PCRE regex | `-E` | `salt -E 'web[0-9]+' test.ping` | Regex match / Регулярное выражение |
| Compound | `-C` | `salt -C 'G@os:SLES and web*' test.ping` | Combined / Комбинированный |
| Nodegroup | `-N` | `salt -N databases test.ping` | Predefined group / Предопределённая группа |
...
salt '<MINION_ID>' salutil.list_states
...
| Service | Unit Name | Port / Порт | Description |
|---------|-----------|-------------|-------------|
...
| MOM | `rhn-search` | — | Search indexer / Индексатор поиска |
...
> [!WARNING]
> After regenerating SSL certificates, all registered minions will need to re-accept the new CA certificate. / После перегенерации SSL-сертификатов все зарегистрированные миньоны должны принять новый CA-сертификат.
...
### Production Backup Runbook / Операционный сценарий резервного копирования
...
### Incremental Content Backup / Инкрементальное резервное копирование

```bash
# Backup script template / Шаблон скрипта резервного копирования
#!/bin/bash
...
# Check saltkey state / Проверить состояние ключей Salt
salt-key -L

# Test connection from minion side / Проверить связь со стороны миньона
# On minion: / На миньоне:
...
> [!TIP]
> Run `sattelite-sync` or `satellite-sync` (depending on version) to re-sync channels if the database and filesystem are out of sync. / Запустите `satellite-sync` для повторной синхронизации каналов, если база данных и файловая система рассинхронизированы.
