...
def clean_header(header_line: str) -> tuple:
    ...
    content = re.sub(r'^##\s+', '', header_line)
    
    if " / " in content:
        parts = content.split(" / ")
        if any(re.search(r'[\u0400-\u04FF]', p) for p in parts[1:]):
            content = parts[0]
            
    content = content.strip()
    return f"## {content}", content

def process_file(filepath: Path, style: str) -> int:
    ...
    if args.files:
        for md_file in args.files:
            if not md_file.is_file():
                print(f"Error: skipping non-existent file {md_file}", file=sys.stderr)
                has_error = True
                continue
            total_count += 1
