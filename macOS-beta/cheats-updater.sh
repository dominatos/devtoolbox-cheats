...
# log_info prints an informational message to standard error.
log_info()  { echo -e "${C_GREEN}[INFO]${C_RESET} $*" >&2; }
# log_warn prints a warning message to standard error.
log_warn()  { echo -e "${C_YELLOW}[WARN]${C_RESET} $*" >&2; }
# log_error writes an error message to standard error with an error prefix and optional color formatting.
log_error() { echo -e "${C_RED}[ERROR]${C_RESET} $*" >&2; }

# cleanup removes the temporary workspace when it exists.
cleanup() {
    [[ -n "${TEMP_DIR:-}" && -d "$TEMP_DIR" ]] && rm -rf "$TEMP_DIR"
}
trap cleanup EXIT

# clone_repo clones the upstream repository into the temporary directory and exits if the clone or required cheats.d directory is unavailable.
clone_repo() {
...
# find_relative_paths lists sorted Markdown file paths relative to the specified directory.
find_relative_paths() {
  local base_dir="$1"
...
# find_full_paths lists sorted full paths to Markdown files under the specified directory.
find_full_paths() {
  local base_dir="$1"
...
# is_not_official_file determines whether a relative path is absent from the official file list.
...
# compat_cp_archive copies files and directories while preserving their attributes across supported platforms.
compat_cp_archive() {
  if [[ "$PLATFORM" == "macos" ]]; then
    cp -rp "$@"
  else
...
# compat_realpath_s resolves a path without requiring the target to exist, using platform-appropriate tools.
compat_realpath_s() {
  local path="$1"
  if [[ "$PLATFORM" == "macos" ]]; then
...
# compat_notify_send displays a desktop notification with the specified title and message when a supported notification service is available. Notification failures are ignored.
compat_notify_send() {
  local title="$1" msg="$2"
  if [[ "$PLATFORM" == "macos" ]]; then
...
# cmd_check compares local cheatsheets with the official repository and reports new, modified, unchanged, and custom files.
cmd_check() {
    clone_repo
    ...
    if [[ ! -d "$CHEATS_DIR" ]]; then
        local total
        total=$(find_full_paths "$TEMP_DIR/cheats.d" | wc -l | tr -d ' ')
...
# cmd_list lists all official cheatsheet paths from the configured repository.
cmd_list() {
    clone_repo
...
# cmd_update synchronizes official cheatsheets, backs up existing files, and rolls back changes if the update or TOC formatting fails.
cmd_update() {
    clone_repo
    ...
    # --- Rollback support: a failed copy or TOC step must not leave a partial update ---
    local added_files=()
    local rolled_back=0
    # rollback_update restores the cheatsheet directory from its backup after a failed update.
    rollback_update() {
        if (( rolled_back )); then return 0; fi
        rolled_back=1
        ...
        if [[ -n "$backup_dir" && -d "$backup_dir" ]]; then
            compat_cp_archive "$backup_dir/." "$CHEATS_DIR/"
            log_error "Original files restored from: $backup_dir"
        fi
...
# show_help displays usage instructions, supported commands, environment configuration, examples, and backup information.
show_help() {
    cat <<EOF
